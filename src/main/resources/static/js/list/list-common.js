const listService = (function() {
    function appendLists(lists) {
        ...
        <div class="list1" title="`;
            ...
            if(`${workList.workStatus}` == '이동중') {
                if(new Date(`${workList.workStartTime}`) > new Date(timestamp)){
            ...
            <li class="usertime"><span>` + formatDateTime(`${workList.workStartTime}`) + `</span> ~ <span>` + formatDateTime(`${workList.workPredictTime}`) + `</span></li>
    ...
    function appendFinishLists(finishLists) {
        finishLists.forEach(workList => {
            ...
            <li class="point2">●</li>
            <li class="username"><span>${workList.memberName}</span> <span>${workList.memberRank}님</span></li>
            <li class="usertime"><span>` + formatDateTime(`${workList.workStartTime}`) + `</span> ~ <span>` + formatDateTime(`${workList.workEndTime}`) + `</span>
            <li class="ow"><span>${workList.workPlaceName}</span></li>
            <li class="work"><span>${workList.workTitle}</li>
        </div>
    `;
