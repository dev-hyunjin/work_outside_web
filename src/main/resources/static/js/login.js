...
function Param() {
    ...
    let memberPassword = $('.member_password').val();
    $.ajax({
        url: '/logins/parameter',
        type: 'get',
        contentType: "application/json; charset=utf-8",
        data: {memberId : memberId , memberPassword: memberPassword},
        success : function (){
            // window.location.href = 'http://localhost:1000/param';
            window.location.href = 'http://localhost:1000/param?memberId='+memberId + '?memberPassword='+memberPassword;
            console.log('성공');
            loginOk();
        },
        error : function (){
