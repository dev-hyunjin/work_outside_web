...
//        teamNumber는 session에 담긴 것 이용하기
            List<StatisticsVO> statisticsList = statisticsService.getStatisticsList(workStartTime, workEndTime, adminService.getMember(sessionNumber).getTeamNumber(), workplaceNumber, memberNumber, criteria);
            int total = statisticsService.getWorkListCount(workStartTime, workEndTime, adminService.getMember(sessionNumber).getTeamNumber(), workplaceNumber, memberNumber);

            List<WorkplaceDTO> workplaceList = adminService.getWorkplaceList("Y", null);
            List<MemberVO> memberList = adminService.getMemberListWithPaging(null, null, null, "Y", null);
...
//        동일, null을 세션에 담긴걸로 처리해야함
    ...
    @GetMapping("/excel")
    public ModelAndView excelStatistics(String workStartTime, String workEndTime, Integer teamNumber, Integer workplaceNumber, Integer memberNumber, HttpSession session, Locale locale) {
